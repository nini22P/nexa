...
import type { BookmarkNode, BookmarkFile } from '../types'
...
interface BookmarkStore {
    ...
    setSort: (key: 'none' | 'title' | 'href' | 'addDate', order: 'asc' | 'desc') => void;
...
const verifyPermission = async (handle: FileSystemHandle, mode: 'read' | 'readwrite' = 'read') => {
...
const useBookmarkStore = create<BookmarkStore>()(
    persist(
        (set, get) => ({
            ...
            updateBookmarkData: (newData) => set((state) => {
            ...
            openFile: async () => {
                try {
                    const [handle] = await window.showOpenFilePicker({
                        types: [{ description: 'HTML Bookmarks', accept: { 'text/html': ['.html', '.htm'] } }],
                        multiple: false
                    })
                    if (!handle) return
                    ...
                    if ((e as Error).name !== 'AbortError') console.error('Open file failed:', e)
                }
            },
            ...
            newFile: async () => {
                try {
                    ...
                    if ((e as Error).name !== 'AbortError') console.error('Create file failed:', e)
                }
            },
            ...
            saveFile: async () => {
                ...
                try {
                    if (!await verifyPermission(bookmarkFile.handle, 'readwrite')) return

                    const fileOnDisk = await bookmarkFile.handle.getFile()
                    const contentOnDisk = await fileOnDisk.text()
                    if (normalize(contentOnDisk) !== normalize(bookmarkFile.lastSavedHtml)) {
            ...
            closeFile: () => {
                ...
                const isDirty = normalize(JSON.stringify(bookmarkFile.data)) !== normalize(bookmarkFile.lastSavedData)
                if (isDirty) {
            ...
            syncWithDisk: async () => {
                ...
                try {
                    ...
                    if ((await bookmarkFile.handle.queryPermission({ mode: 'read' })) !== 'granted') return
                    ...
                    if (diskDataStr !== bookmarkFile.lastSavedData) {
                        const isUserDirty = normalize(JSON.stringify(bookmarkFile.data)) !== normalize(bookmarkFile.lastSavedData)
                        if (!isUserDirty) {
                            set({
                                bookmarkFile: {
                                    ...bookmarkFile,
            ...
            addItem: (type, parentId) => {
                ...
                const newItem: BookmarkNode = type === 'folder'
                    ? { id, parentId, type: 'folder', title: '新文件夹', addDate: now, lastModified: now, childrenIds: [] }
                    : { id, parentId, type: 'link', title: '新书签', href: 'https://', addDate: now, lastModified: now }
            ...
            updateItem: (id, updates) => {
                ...
                const newData = {
                    ...bookmarkFile.data,
                    [id]: { ...bookmarkFile.data[id], ...updates, lastModified: Date.now().toString() }
                }
                set({ bookmarkFile: { ...bookmarkFile, data: newData } })
            },
            ...
            deleteItem: (id) => {
                ...
                if (nodeToDelete.parentId && newData[nodeToDelete.parentId]) {
                    newData[nodeToDelete.parentId] = {
                        ...newData[nodeToDelete.parentId],
                        childrenIds: newData[nodeToDelete.parentId].childrenIds?.filter(childId => childId !== id)
                    }
                }
            ...
            getCurrentItems: () => {
                const { bookmarkFile, activeFolderId, searchQuery, sortKey, sortOrder } = get()
                if (!bookmarkFile) return []
                ...
                return [...items].sort((a, b) => {
                    const valA = (a[sortKey as keyof BookmarkNode] || '').toString().toLowerCase()
                    const valB = (b[sortKey as keyof BookmarkNode] || '').toString().toLowerCase()
