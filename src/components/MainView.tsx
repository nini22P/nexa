...
export default function MainView() {
    ...
    const activeFolderName = isSearching
        ? '搜索结果'
        : (activeFolderId === null ? '全部书签' : (activeFolder?.title || '未知文件夹'))
    ...
    return (
            ...
            <header className="flex-none p-4 bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-xl font-bold text-slate-900 truncate">{activeFolderName}</h1>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <div className="relative group flex-1 md:flex-none md:w-64">
                            <i className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-slate-900 transition-colors text-lg">search</i>
                            <input
                                type="text"
                                placeholder="搜索书签或网址..."
                        ...
                        <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-xl">
                            <select
                                ...
                                value={sortKey}
                                onChange={(e) => setSort(e.target.value as 'none' | 'title' | 'href' | 'addDate', sortOrder)}
                            >
                            ...
                            </select>
                            {
                                sortKey !== 'none'
                                    ? (
                                        ...
                                        >
                                            <i className="material-symbols-outlined text-base">{sortOrder === 'asc' ? 'arrow_upward' : 'arrow_downward'}</i>
                                        </button>
                                    )
                                    : <div className='w-1' />
                            }
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                <i className="material-symbols-outlined text-base text-slate-500">create_new_folder</i>
                                <span>新建文件夹</span>
                            </button>
                            <button
                                onClick={() => {
                ...
                {currentItems.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-2">
                        {currentItems.map((item) => {
                            return (
                                <div key={item.id} className="group animate-in fade-in zoom-in-95 duration-300">
                                    <article
                                        className={`
        ...
        `}
                                    ...
                                    >
                                        <div className={`w-10 h-10 flex items-center justify-center rounded-lg flex-none transition-colors ${item.type === 'folder'
                                                ...
                                                <img
                                                    ...
                                                    onError={(e) => {
                                                        ...
                                                        if (target.parentElement) {
                                                            target.parentElement.innerHTML = '<i class="material-symbols-outlined text-xl">link</i>'
                                                        }
                                                    }}
                                                />
                                            )}
